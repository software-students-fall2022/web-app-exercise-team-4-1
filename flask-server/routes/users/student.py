...
def get_students(student_ids):
    courses = Database.find(
        "Student", {"_id": {"$in": [ObjectId(id) for id in student_ids]}})
    return loads(dumps(courses))


def get_student_courses():
    courses = Database.find_single("Student", {"username": views.username})[
        'course_list']
...
def add_to_cart():
    ...
    if Database.count("Student", {"username": views.username, "carts": {"$in": [ObjectId(section_id)]}}) == 0:
        Database.update("Student", {"username": views.username}, {
                        '$push': {'carts': ObjectId(section_id)}})
...
def get_cart():
    carts = Database.find_single("Student", {"username": views.username}, {
                                 '_id': 0, 'carts': 1})['carts']
