...
def get_courses(courses_id):
    courses = Database.find(
        "Course", {"_id": {"$in": [ObjectId(id) for id in courses_id]}})
...
def search_course(search):
    ...
    result = Database.find(
        "Course", {"name": re.compile(search, re.IGNORECASE)})
    return loads(dumps(result))
    ...
    if (views.admin):
        ...
        if (course_count == 0):
            ...
            if (add_student(student_oid, course_id, section_id)):
                ...
                views.displayMsg = 'Student is on waitlist'
                views.render = '/add_student'
                return redirect(url_for('app_blueprint.add_student_view', course_id=course_id, section_id=section_id))
        else:
        ...
        course_count = Database.count("Student", {
                                      "username": views.username, "course_list": {"$in": [ObjectId(course_id)]}})
        if (course_count == 0):
            student_oid = Database.find_single(
                "Student", {"username": views.username})['_id']
            Database.update("Student", {"username": views.username}, {
                            '$pull': {'carts': ObjectId(section_id)}})
            if (add_student(student_oid, course_id, section_id)):
                Database.update("Student", {"username": views.username}, {
                ...
                views.displayMsg = "Course is on waitlist"
                views.isError = False
                views.render = '/cart'
                return redirect(url_for('app_blueprint.shopping_cart_view'))
        else:
...
def update_course(course_id):
    ...
    if (courseID and name and description):
        result = Database.update("Course", {"_id": ObjectId(course_id)}, {
                                 "$set": {'courseID': courseID, 'description': description, 'name': name}})
...
def update_add_waitlist(studentId, course_id, section_id):
    if Database.count("Course", {'sections': {'$elemMatch': {'_id': ObjectId(section_id), 'waitlist': ObjectId(studentId)}}}) == 0:
        Database.update("Course", {"_id": ObjectId(course_id), "sections._id": ObjectId(
            section_id)}, {'$push': {'sections.$.waitlist': ObjectId(studentId)}})
        return True
    else:
        return False
...
def delete_course(course_id):
    ...
    for student in student_ids:
        Database.update("Student", {"_id": ObjectId(student)}, {
...
def add_student(studentId, course_id, section_id):
    ...
    if (counts >= capacity):
        ...
        Database.update("Course", {'_id': ObjectId(course_id), "sections._id": ObjectId(
            section_id)}, {'$push': {"sections.$.student": ObjectId(studentId)}})
        return True


def remove_student(studentId, courseId, sectionId):
    Database.update("Course", {'_id': ObjectId(courseId), "sections._id": ObjectId(
        sectionId)}, {'$pull': {"sections.$.student": ObjectId(studentId)}})
    update_remove_waitlist(courseId, sectionId)


def remove_sections(courseId, sectionId):
    Database.update("Course", {"_id": ObjectId(courseId)}, {
                    '$pull': {'sections': {'_id': ObjectId(sectionId)}}})

    students = list(get_all_students())
    student_ids = [student['_id'] for student in students]
    for student in student_ids:
        Database.update("Student", {"_id": ObjectId(student)}, {
                        '$pull': {'carts': ObjectId(sectionId)}})
        Database.update("Student", {"_id": ObjectId(student)}, {
                        '$pull': {'course_list': ObjectId(courseId)}})
